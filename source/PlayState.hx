...
class PlayState extends FlxState
{
    ...
    var _maskCam:FlxShaderMaskCamera;

	override public function create():Void
	{
        // replace default camera with the ShaderMask camera we want to test
        _maskCam = new FlxShaderMaskCamera(new OpenFLInvertShader(), 0, 0, FlxG.width, FlxG.height);
        FlxG.cameras.reset(_maskCam);
	...
	function changeBunnyNumber(add:Bool, amount:Int):Void
	{
        ...
        // Any time we add or remove normal bunnies, also change the number of
        // mask bunnies proportionally
        if (amount > 10) {
            for(i in 0...Std.int(amount/10)) {
                if(add)
                {
                    var maskBunny = new Bunny();
                    maskBunny.loadGraphic("assets/wabbit_mask.png");
                    maskBunny.init(offScreen);
                    _maskCam.addMaskSprite(maskBunny);
                }
                else
                {
                    var maskBunny = _maskCam.maskSprites.getFirstAlive();
                    if (maskBunny != null)
                        _maskCam.maskSprites.remove(maskBunny);
                }
            }
        }
